...
function run_arangoproxy_and_site() {
  ...
  if [[ -z "${HUGO_URL}" ]]; then
    HUGO_URL="http://localhost"   ## dev as default env
  fi

  docker run -d --name site --network=docs_net --ip=192.168.129.130 --env-file toolchain/docker/env/"$DOCKER_ENV".env -e HUGO_URL="$HUGO_URL" -p 1313:1313 --volumes-from toolchain --log-opt tag="{{.Name}}" site 
