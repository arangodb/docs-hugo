...
| **Standalone ArangoDB**, offline / air-gapped | [Generate a license key](#generate-a-license-key) on a separate internet-connected machine, then [apply it](#apply-a-license-key) via _arangosh_, the web interface, or the HTTP API. | **Yes** — on the internet-connected machine only |
...
- Running **standalone ArangoDB** (no Kubernetes) and want a license key
  file you can apply via _arangosh_ or the web interface, or
