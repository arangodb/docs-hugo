...
- the HTTP API for fetching the list of currently running AQL queries and the HTTP API
...
- The HTTP API for retrieving indexes (GET /_api/index) now returns the *deduplicate*
  attribute for each index

- The HTTP API for creating indexes (POST /_api/index) now accepts the optional *deduplicate*
  attribute

- The HTTP API for executing a server-side transaction (POST /_api/transaction) now accepts the optional attributes: `maxTransactionSize`, `intermediateCommitCount`, `intermediateCommitSize`

- The HTTP API for creating a cursor (POST /_api/cursor) now accepts the optional attributes: `failOnWarning`, `maxTransactionSize`, `maxWarningCount`, `intermediateCommitCount`, `satelliteSyncWait`, `intermediateCommitSize`. `skipInaccessibleCollections`
...
- It is no longer supported to access the `_users` collection in any way directly, except through the official `@arangodb/users` module or the `/_api/users` HTTP API.

- The access to the `_users` collection from outside of the _arangod_ server process is now forbidden (Through drivers, _arangosh_ or the HTTP API). Foxx services are still be able to access the `_users` collection for now, but this might change in future minor releases.
