...
| `files[].citable_url` | string | No | URL that citations of this document link to, for files imported here. Files a corpus build pulls from File Manager instead carry their own, set as [custom metadata](../../../platform-suite/file-manager/api.md#the-citable_url-key) at upload. Citations are not extracted from document text automatically; see [Known Limitations](error-handling.md#citation-handling). | Provide the source URL for web-sourced documents (for example, `https://docs.example.com/guide`). Omit the field for documents without a canonical web location. |
