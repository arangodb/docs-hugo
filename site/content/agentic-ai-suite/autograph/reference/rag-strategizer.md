...
**Recommended path:** Optional audit step after `POST /v1/rag-strategizer/analyze` completes. Use it to check which strategy each cluster was assigned before you orchestrate, and to get the `rag_partition_id` values that `POST /v1/graph/recluster` takes. Safe to call anytime for read-only inspection.
