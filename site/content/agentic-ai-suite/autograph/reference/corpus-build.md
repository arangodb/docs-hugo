...
| `STORAGE_FILE_TOO_LARGE` | `completed` | A file was larger than the entire local staging space and could therefore never be downloaded, so it was skipped. The remaining files were still processed. Ordinary exhaustion of the staging space is expected on a large build and drops nothing: files are downloaded in waves, and each wave is processed and deleted before the next one starts, so a build of several gigabytes fills and frees the space many times over. | `message` names the skipped File Manager IDs, the first five, then `, ... (N more)`. Split or shrink those files, or ask your operator to raise `LOCAL_STORAGE_MAX_BYTES` past the size of the largest single file. |
