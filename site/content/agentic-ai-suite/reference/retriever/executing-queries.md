...
{{< endpoint "POST" "https://<EXTERNAL_ENDPOINT>:8529/_platform/acp/v1/graphrag-query" >}}

{{< endpoint "POST" "https://<EXTERNAL_ENDPOINT>:8529/_platform/acp/v1/graphrag-query-stream" >}}

{{< tip >}}
The Instant Search endpoint (`/_platform/acp/v1/graphrag-query-stream`) returns streaming responses, 
...
```bash
curl -X POST /_platform/acp/v1/graphrag-query-stream \
...
```bash
curl -X POST /_platform/acp/v1/graphrag-query \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST https://<EXTERNAL_ENDPOINT>:8529/_platform/acp/v1/graphrag-query-stream \
  -H "Content-Type: application/json" \
  -d '{
    "query": "How are X and Y related?",
    "query_type": "UNIFIED",
...
```bash
curl -X POST https://<EXTERNAL_ENDPOINT>:8529/_platform/acp/v1/graphrag-query \
  -H "Content-Type: application/json" \
  -d '{
